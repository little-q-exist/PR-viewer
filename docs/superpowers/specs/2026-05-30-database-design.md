...
基于 GitHub App 认证模式的端点调整：

| 原端点 | 调整后 | 说明 |
|--------|--------|------|
| `POST /auth/login` | `POST /auth/install` | 接收 installation code，返回 JWT |
| — | `GET /auth/me` | 新增：获取当前用户信息 |
| `GET /pull-requests` | 不变 | 返回已缓存的 PR 列表 |
| `GET /pull-requests/{id}` | 不变 | PR 详情 |
| `POST /reviews` | 不变 | Body: `{ prUrl: string }` |
| `GET /reviews/{id}` | 不变 | 评审结果（含 summary + fileAnalyses） |
| `GET /reviews` | 不变 | 评审列表，支持 `?status=` 和分页 |
